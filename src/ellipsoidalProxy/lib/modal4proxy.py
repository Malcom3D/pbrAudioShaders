...
class Modal4Proxy:
    ...
    def compute(self, obj_idx: int) -> None:
        ...
        if proxy_type in [0, 1, 2]:
            ...
            debug_print(f"Approximate modal model for {config_obj.name} proxy_type {proxy_type}")
...
#        center_local = (min_coords + max_coords) / 2
    ...
    def _save_faust_lib(
        ...
        if config_obj.resonance:
            ...
            header = f"Resonance modal model for {config_obj.name} (proxy)"
            
            resonance_content = _generate_lib(header=header, generator=generator, output_name=f"{output_name}_resonance", n_modes=resonance_modes, n_vertices=n_vertices, freq_str=resonance_freq_str, t60_str=resonance_t60_str, gain_waveform=resonance_gain_str)
